...
lstm1 = LSTM(64, return_sequences=True)(conv2)
lstm2 = LSTM(32)(lstm1)
drop_mfcc = Dropout(0.4)(lstm2)
flatten_mfcc = Flatten()(drop_mfcc)


# Build Dialogue Embedding processing path (using Transformer for better text representation)
embedding_input = Input(shape=(None, 384), name="dialogue_embedding")
transformer_attention = MultiHeadAttention(num_heads=4, key_dim=384)(embedding_input, embedding_input)
transformer_attention = LayerNormalization()(transformer_attention)
transformer_attention = Dropout(0.4)(transformer_attention)
text_lstm1 = Bidirectional(LSTM(64, return_sequences=True))(transformer_attention)
text_lstm2 = Bidirectional(LSTM(32))(text_lstm1)
drop_text = Dropout(0.3)(text_lstm2)
flatten_text = Flatten()(drop_text)

flatten_mfcc = Dense(64, activation="relu")(flatten_mfcc)  # Adjust dimension to match flatten_text
flatten_text = Dense(64, activation="relu")(flatten_text)  # Ensure both inputs are of the same size
...
# Model Assembly
model = Model(inputs=[mfcc_input, embedding_input], outputs=output)
